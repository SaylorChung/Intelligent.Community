...
namespace Saylor.Specifications
{
    /// <summary>
    ...
    public static class ExpressionExtension
    {
        ...
        public static Expression<Func<T, dynamic>> Sort<T>(string propertyName)
        {
            var property = typeof(T).GetProperty(propertyName);
            var parameter = Expression.Parameter(typeof(T), "p");//参数
            var propertyAccess = Expression.MakeMemberAccess(parameter, property);

            return Expression.Lambda<Func<T, dynamic>>(Expression.Convert(propertyAccess, typeof(object)), parameter);
        }
        #endregion
    }
}
